...
UTest_InventoryComponent::UTest_InventoryComponent()
{
	PrimaryComponentTick.bCanEverTick = true;
	
}

void UTest_InventoryComponent::BeginPlay()
{
	Super::BeginPlay();
}


void UTest_InventoryComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);
}

bool UTest_InventoryComponent::SwitchWeaponByIndex(int32 IndexWeaponToChange, int32 PreviousIndex, FAdditionalWeaponInfo PreviousWeaponInfo)
{
...
FAdditionalWeaponInfo UTest_InventoryComponent::GetAdditionalInfoWeapon(int32 IndexWeapon)
{
...
int32 UTest_InventoryComponent::GetWeaponIndexSlotByName(FName IdWeaponName)
{
	int32 Result = -1;
	int8 i = 0;
	bool bIsFind = false;
...
FName UTest_InventoryComponent::GetWeaponNameBySlotIndex(int32 indexSlot)
{
	FName Result;
...
bool UTest_InventoryComponent::GetWeaponTypeByIndexSlot(int32 IndexSlot, EWeaponType& WeaponType)
{
	bool bIsFind = false;
	WeaponType = EWeaponType::RifleType;
	UTest_GameInstance* MyGI = Cast<UTest_GameInstance>(GetWorld()->GetGameInstance());
	if (MyGI)
	{
		if (WeaponSlots.IsValidIndex(IndexSlot))
		{
...
bool UTest_InventoryComponent::GetWeaponTypeByNameWeapon(FName IdWeaponName, EWeaponType& WeaponType)
{
	bool bIsFind = false;
	WeaponType = EWeaponType::RifleType;
	UTest_GameInstance* MyGI = Cast<UTest_GameInstance>(GetWorld()->GetGameInstance());
...
void UTest_InventoryComponent::SetAdditionalInfoWeapon(int32 IndexWeapon, FAdditionalWeaponInfo NewInfo)
{
	if (WeaponSlots.IsValidIndex(IndexWeapon))
	{
		bool bIsFind = false;
...
void UTest_InventoryComponent::AmmoSlotChangeValue(EWeaponType TypeWeapon, int32 CountChangeAmmo)
{
	bool bIsFind = false;
	int8 i = 0;
	while (i < WeaponSlots.Num() && !bIsFind)
...
bool UTest_InventoryComponent::CheckAmmoForWeapon(EWeaponType TypeWeapon, int8& AvailableAmmoForWeapon)
{
...
bool UTest_InventoryComponent::CheckCanTakeAmmo(EWeaponType AmmoType)
{
	bool Result = false;
	int8 i = 0;
...
bool UTest_InventoryComponent::CheckCanTakeWeapon(int32& FreeSlot)
{
...
void UTest_InventoryComponent::TryGetWeaponToInventory(AActor* PickUpActor, FWeaponSlot NewWeapon)
{
	int32 IndexSlot = -1;
...
TArray<FWeaponSlot> UTest_InventoryComponent::GetWeaponSlots()
{
	return WeaponSlots;
}

void UTest_InventoryComponent::InitInventory(const TArray<FWeaponSlot>& NewWeaponSlotsInfo)
{
...
void UTest_InventoryComponent::SwitchWeaponEvent(FName WeaponName, FAdditionalWeaponInfo AdditionalInfo, int32 IndexSlot)
{
	OnSwitchWeapon.Broadcast(WeaponName, AdditionalInfo, IndexSlot);
}

void UTest_InventoryComponent::WeaponAdditionalInfoChangeEvent(int32 IndexWeapon, FAdditionalWeaponInfo AdditionalWeaponInfo)
{
	OnWeaponAdditionalInfoChange.Broadcast(IndexWeapon, AdditionalWeaponInfo);
}

void UTest_InventoryComponent::AmmoChangeEvent(EWeaponType TypeWeapon, int32 Count)
{
